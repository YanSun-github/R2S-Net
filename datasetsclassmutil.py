...
class LOSO_DATASET(data.Dataset):
    def __init__(self, opt,input, split, subject):
        super().__init__()
        self._split = split
        self._subject = subject
        self._segment_feat_root = input#opt['segment_feat_root']
    ...
    def __getitem__(self, index):
        ...
        # csv_file = "/code/CodeTest/wzl/AUW-GCN/updated_target.csv"
        # df_csv = pd.read_csv(csv_file)
        # df_csv = df_csv[df_csv.iloc[:, 1] == vid_name]
        STEP = self._step
        ...
        micro_class_slabels = labels[:, 8]
        macro_class_slabels = labels[:, 9]
        ...
        micro_class_label = [0] * (self._segment_len + STEP * 2)  # 0->neural, 1 2 3, 4->others
        macro_class_label = [0] * (self._segment_len + STEP * 2)  #
        ...
        #对峰值帧周围做标记micro_normal_range范围内为一个正态分布
        # apex (micro)
        ...
        for index, action_label in enumerate(micro_action_labels):
            if action_label == 1:
                ...
                # if micro_class_slabels[index]==2:
                #     micro_class_label[index] = 0
                # else:

                micro_class_label[index] = micro_class_slabels[index]
            # else:
            #     micro_class_label[index]=2
            #micro_class_label[index]=micro_class_slabels[index]
        for index, action_label in enumerate(macro_action_labels):
            if action_label == 1:
                macro_action_score[index] = 1
                # if macro_class_slabels[index] == 2:
                #     macro_class_label[index] = 0
                # else:

                macro_class_label[index] = macro_class_slabels[index]
            # else:
            #     macro_class_label[index] = 2
        ...
        # micro_class_score=micro_class_label[STEP:-STEP]
        # macro_class_score = macro_class_label[STEP:-STEP]
        ...
        micro_class_score = torch.tensor(micro_class_label, dtype=torch.int64)
        macro_class_score = torch.tensor(macro_class_label, dtype=torch.int64)

        #0->2 1 2wu 3
        if self._split == "train":
            return (feature, micro_apex_score, macro_apex_score,
                    micro_action_score, macro_action_score,
                    micro_start_end_label, macro_start_end_label,micro_class_score,macro_class_score)
